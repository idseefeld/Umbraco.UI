...
export function snapshotColorStyles() {
  Array.from(document.querySelectorAll('[data-sketch-color]')).forEach(node => {
...
export function snapshotTextStyles({ suffix = '' }) {
  Array.from(document.querySelectorAll('[data-sketch-text]')).forEach(node => {
    getAllLayers(node)
